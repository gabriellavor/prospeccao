<?php
	...
	class LinkedinController extends Controller{
        ...
        public function retornaLinkedin($request,$response,$args){
            ...
            var_dump($_SESSION);
            if(!empty($_SESSION['info_linkedin'])){
                $retorno = $_SESSION['info_linkedin'];
            }
            //return $retorno;
        }
        
	
		public function loginLinkedin($request,$response,$args){
			header('Access-Control-Allow-Origin: *');
			header('Access-Control-Allow-Methods: GET');
			header('Access-Control-Allow-Headers: token, Content-Type');
			header('Access-Control-Allow-Credential: true');
			...
			$li = new LinkedIn(array(
				'api_key' => '77sc9udgaee7qe', 
				'api_secret' => 'RcECJxPlU3WggW6z', 
                //'callback_url' => 'https://scorelead.com.br/home',
                //'callback_url' => 'http://localhost:8080/home',
                'callback_url' => 'http://localhost:8080/linkedin'
			));
			$url = $li->getLoginUrl(
				array(
            ...
            $token = $li->getAccessToken($code['code']);
            $token_expires = $li->getAccessTokenExpiration();
            $info = $li->get('/people/~:(first-name,last-name,positions,picture-url)');
            session_start();
            $_SESSION['info_linkedin'] = json_encode($info);
            $_SESSION['info'] = json_encode($res);
            header("location:http://localhost:4200/login-linkedin/login-linkedin");
            die();
		}
	}
